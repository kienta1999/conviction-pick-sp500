...
Rebalances account **U27177562** toward `picks/picks_dip_<date>.csv`, built from
the newest dip `rank10` run in `picks/ledger.csv` (the row set the panel wrote
from `output/dip/final_ranking.md`). Rules live in **POLICY.md §6** — read it
first; this skill executes it and never re-argues it.
...
| | default (POLICY.md §6) |
|---|---|
| picks | `uv run python scripts/ranking_to_picks.py` → newest `picks/picks_dip_*.csv` |
| scheme | `equal3` (`--scheme equal10` etc. only if the user says so) |
...
```bash
uv run python scripts/ranking_to_picks.py            # prints the basket + path
PICKS=$(ls -t picks/picks_dip_*.csv | head -1)
```

Show the user the basket. If it is not the run they expect (date in the
filename), stop.

`FLAGS="--broker web --mode <mode> --leverage 1.0 --vol-target 0.2 --picks $HERE/$PICKS"`.
Every script call below is
...
### 4. Capture the account

```bash
$EXEC --dump-tickers | tail -1
```

Read `$JS/capture_state.js`, replace `__ACCOUNT__` and `__TICKERS__` (JS array
from the line above), run with `browser_evaluate` using
`filename: "web_account.json"`, then:
...
**sold** because it left the top 3, append a `kind=close` row
(`exit_reason=rebalance`, `exit_price` = fill). Commit `picks/` and
